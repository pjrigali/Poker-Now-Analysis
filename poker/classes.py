...
class Hand:
    ...
    def __init__(self, lst_hand_objects: list, file_id: str, player_dic: dict):
        ...
        for line in self._parsed_hand:
            ...
            if line.player_index is not None and self._starting_player_chips is not None and line.player_index in self._starting_player_chips.keys():
            ...
            if line_type == SmallBlind:
                ...
                if line.cards is not None:
                    _add_to_dic(item=line.cards, player_dic=player_dic, location='Cards', player_index=line.player_index)
                    _add_to_dic(item=line.cards, player_dic=player_dic, location='Cards', player_index='Win')
                _add_to_dic(item=line.winning_hand, player_dic=player_dic, location='Hands', player_index=line.player_index)
            ...
            if winner.cards is None:
                for temp_line in self.parsed_hand:
                    if type(temp_line) == Shows and temp_line.player_index == winner.player_index:
                        ...
                        _add_to_dic(item=temp_line.cards, player_dic=player_dic, location='Cards', player_index=temp_line.player_index)
                        break
...
def _line_to_df(line_lst: list, player_index: List[str]) -> pd.DataFrame:
    lst = []
    ...
    for key in dic.keys():
        if key in ['Flop', 'Turn', 'River', 'Win', 'My Cards']:
            if key in ['Flop', 'Win', 'My Cards']:
                ...
                lst = dic[key]['Cards']
            wrong = []
            temp = [i if type(i) == str else wrong.append(i) for i in lst]
            temp += sum(wrong, [])
            card_count_dic[key + ' Count'] = dict(Counter(temp))
...
class Game:
    ...
    def __init__(self, hand_lst: List[str], file_id: str, players_data: dict):
        ...
        player_dic = {}
        self._parsed_hands = [Hand(lst_hand_objects=[line for line in parser(hand=hand)], file_id=file_id, player_dic=player_dic) for hand in hand_lst]

        player_info_dic = {}
        for key in player_dic.keys():
            if key not in ['Flop', 'Turn', 'River', 'Win', 'My Cards']:
                player_info_dic[key] = player_dic[key]
...
def _build_player_dic(data: dict, matches: List[Game]) -> dict:
    player_dic = {}
...
def _get_dist(matches: List[Game]) -> List[pd.DataFrame]:
    hand_ind = set(sum([list(match.winning_hand_distribution.keys()) for match in matches], []))
